...
```{r values, echo = FALSE, message = FALSE}
...
#### For the next function we wanted to see which team had the most concussion, which we found to be `r concussions_by_team` We thought this would be a valuable value as it would allow us to see if teams were doing dramatically better than others with concussion prevention. It turned out that the Bengals and Browns had the most concussion and those teams should assess their concussion prevention protocol.

#### For this function we wanted to see which position had the most concussions, which we found to be `r concussions_by_position` We thought it would be valuable to look at if certain positions were “more dangerous” than others. It turned out that cornerback was the most dangerous position. This data should shape people's decision of what position to play.

#### For this function we wanted to see which month had the most concussions, which we found to be `r concussions_by_month` We thought it would be valuable to look at if certain months were potentially the most dangerous and draw conclusions based on that.The highest month was September, maybe due to players hitting harder at the start of the season. The league should look to more tightly enforce unnecessary roughness penalties at the start of the season.

#### For this last function we wanted to see if there was a connection between pre-season injuries and concussion. We thought it would be valuable to look at because re-injury is often a problem, especially with head injuries. It turned out to be only `r had_preseason_injury` of players who were concussed had an injury in the pre-season.
